...
class Food {
  ...
  static async updateFoodByID(id, newFood) {
    const food = await Food.getDrugByID(id);
    if (!food) return null;
    const updatedData = { ...food, ...newFood };
    const { rows } = await pool.query(`
      UPDATE drugs
      SET grocery_name = $2, common_name = $3, family = $4, plant_name = $5, is_it_good = $6
      WHERE id = $1
      RETURNING *
    `,
    [id, updatedData.grocery_name, updatedData.common_name, updatedData.family, updatedData.plant_name, updatedData.is_it_good]
    );
    return new Food(rows[0]);
  }
